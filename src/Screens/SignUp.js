            ...
            if (password == conformPassword) {
                ...
                firebase
                    ...
                    .then(response => {
                        ...
                        this.setState({ isLoading: false, isSuccessful: true, fullName: '', email: '', password: '', conformPassword: '' });
                        setTimeout(() => {
                            console.log("isSuccessfulInitial::", this.state.isSuccessful, this.state.isLoading)
                            this.setState({ isSuccessful: false })
                            //Alert.alert('Congrats', "You've logged in Successfully!");
                            console.log("isSuccessful::", this.state.isSuccessful, this.state.isLoading)
                        }, 2000)
                    })
            } else {
                ...
                <KeyboardAwareScrollView>
                    <SafeAreaView>
                        <View style={{ alignItems: 'center', marginTop: Platform.OS === 'ios' ? hp('3%') : hp('5%') }}>
                            ...
                            <Text style={{ fontSize: 12, fontWeight: '500', color: '#121212' }}>Create an account to Krishi Seva Bandhu to get all features</Text>
                        </View>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: Platform.OS === 'ios' ? hp('5%') : hp('3%'), height: Platform.OS === 'ios' ? hp('6.5%') : hp('8%'), borderRadius: 30, borderWidth: 1, borderColor: '#53E0BC', alignSelf: 'center', width: Dimensions.get('window').width - 30 }}>
                            <View style={{ flex: 0.2, alignItems: 'center' }}>
                                <Icon name='ios-lock-closed' size={30} color='#53E0BC' />
                            </View>
                            <TextInput style={{ flex: 0.8, fontSize: 22, color: '#53E0BC' }}
                                autocapitalize='none'
